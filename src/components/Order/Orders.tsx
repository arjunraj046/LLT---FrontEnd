...
import ImportedOrderList from './ImportedOrderList';
import ImportOrder from './ImportOrder';

const Entries: React.FC = () => {
  const [activeTab, setActiveTab] = useState<
    'ImportOrder'|'orderList' | 'cumulativeEntries' | 'entityList'
  >('ImportOrder');

  const handleTabChange = (
    tab: 'ImportOrder'|'orderList' | 'cumulativeEntries' | 'entityList',
  ) => {
    setActiveTab(tab);
  };
  ...
  return (
    <div>
      <div className="text-sm font-medium text-center text-gray-500 border-b border-gray-200 dark:text-gray-400 dark:border-gray-700">
        ...
        <li className="me-2">
            <button
              ...
              onClick={() => handleTabChange('ImportOrder')}
            >
              Add Order
            </button>
          </li>
      ...
      <div>
      {activeTab === 'ImportOrder' && <ImportOrder />}
      {activeTab === 'orderList' && <ImportedOrderList />}
        {activeTab === 'entityList' && <TokenList />}
        {activeTab === 'cumulativeEntries' && <Cumulative />}
